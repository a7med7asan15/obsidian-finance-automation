...
var CATEGORY_RULES_PATH = `${CATEGORIES_DIR}/rules.md`;
...
function expandYear(year) {
  return year.length === 2 ? String(2e3 + Number(year)) : year;
}
...
var DEFAULT_CATEGORY_RULES = {
  rules: DEFAULT_CATEGORIES.map((category) => ({
    category: category.name,
    keywords: [...category.keywords]
  }))
};
var heading = (title, body) => `# ${title}
...
var BY_PATH = new Map(SETTINGS_FILES.map((file) => [file.path, file]));
function introFor(path) {
...
var JSON_BLOCK = /(`{3,})json[ \t]*\r?\n([\s\S]*?)\r?\n?\1/;
function legacyJsonPath(path) {
  return path.endsWith(".md") ? `${path.slice(0, -3)}.json` : path;
}
function resolveSettingsPath(app, path) {
  const note = (0, import_obsidian3.normalizePath)(path);
  if (app.vault.getAbstractFileByPath(note)) return note;
  const legacy = (0, import_obsidian3.normalizePath)(legacyJsonPath(path));
  if (legacy !== note && app.vault.getAbstractFileByPath(legacy)) return legacy;
  return note;
}
function parseSettingsContent(path, content) {
  const text = content ?? "";
...
function withJsonBlock(existing, json) {
...
async function loadVaultJson(app, path, fallback) {
  ...
  try {
    return parseSettingsContent(resolved, await app.vault.cachedRead(file));
  } catch (error) {
...
async function saveVaultJson(app, path, value) {
  ...
  if (existing instanceof import_obsidian3.TFile) {
    await app.vault.process(
      existing,
      (current) => resolved.endsWith(".md") ? withJsonBlock(current, json) : `${json}
`
    );
    return;
  }
...
async function createAccountNote(app, draft) {
  const path = (0, import_obsidian7.normalizePath)(accountNotePath(draft.name));
  if (app.vault.getAbstractFileByPath(path)) throw new Error(`${path} already exists.`);
...
async function renameAccountNote(app, path, name) {
  ...
  const wanted = (0, import_obsidian7.normalizePath)(accountNotePath(name));
  if (wanted !== file.path) {
    if (app.vault.getAbstractFileByPath(wanted)) throw new Error(`${wanted} already exists.`);
    await app.fileManager.renameFile(file, wanted);
  }
  const moved = app.vault.getAbstractFileByPath(wanted);
  if (!(moved instanceof import_obsidian7.TFile)) throw new Error(`Could not find ${wanted} after renaming.`);
  await app.fileManager.processFrontMatter(moved, (frontmatter) => {
    frontmatter.name = String(name).trim();
  });
  return wanted;
...
var INBOX_README = `${INBOX_DIR}/README.md`;
var INBOX_README_BODY = `# Inbox
...
function workspaceSeeds(app) {
  const seeds = [{ path: INBOX_README, content: INBOX_README_BODY }];
  for (const file of SETTINGS_FILES) {
    const json = JSON.stringify(file.content, null, 2);
    const legacy = (0, import_obsidian8.normalizePath)(legacyJsonPath(file.path));
...
${withJsonBlock("", json)}` });
  ...
  return seeds.map((seed) => ({ ...seed, path: (0, import_obsidian8.normalizePath)(seed.path) }));
}
async function planWorkspace(app) {
  const plan = { folders: [], create: [], reset: [] };
  ...
  for (const seed of workspaceSeeds(app)) {
    ...
    else if (existing instanceof import_obsidian8.TFile && await app.vault.cachedRead(existing) !== seed.content) {
      plan.reset.push(seed.path);
    }
  }
  return plan;
}
async function ensureWorkspace(app) {
...
async function writeSeed(app, result, path, content) {
...
function describeWorkspace(result) {
  const reset = result.replaced.length ? `reset ${result.replaced.length} file${result.replaced.length === 1 ? "" : "s"} to the defaults` : "";
  if (!result.created.length) {
    return reset ? `Budget: ${reset}.` : `Budget: the ${VAULT_ROOT} files are already in place.`;
  }
  const made = result.unchanged.length && result.created.length <= 4 ? `created ${result.created.join(", ")}` : `created ${VAULT_ROOT} \u2014 inbox, transactions, settings, ${DEFAULT_ACCOUNTS.length} accounts and ${DEFAULT_CATEGORIES.length} categories`;
  return `Budget: ${[made, reset].filter(Boolean).join(", and ")}.`;
}

// src/ui/workspace-setup.ts
var LISTED = 10;
async function setUpWorkspace(app) {
  let plan;
  try {
    plan = await planWorkspace(app);
  } catch (error) {
    new import_obsidian9.Notice(`Budget: could not read the ${VAULT_ROOT} files \u2014 ${error.message}`, 8e3);
    return;
  }
...
async function applyWorkspace(app) {
  try {
    new import_obsidian9.Notice(describeWorkspace(await ensureWorkspace(app)), 8e3);
  } catch (error) {
    new import_obsidian9.Notice(`Budget: could not create the files \u2014 ${error.message}`, 8e3);
  }
}
var WorkspaceResetModal = class extends import_obsidian9.Modal {
  constructor(app, plan, confirm) {
    super(app);
    this.plan = plan;
  ...
  onOpen() {
    ...
    contentEl.createEl("p", {
      text: `It also puts ${reset.length} file${reset.length === 1 ? "" : "s"} back to ${reset.length === 1 ? "its default" : "their defaults"}, replacing what ${reset.length === 1 ? "it holds" : "they hold"} now \u2014 including any budget, colour, card ending or keyword on ${reset.length === 1 ? "it" : "them"}:`
    });
    ...
    new import_obsidian9.Setting(contentEl).addButton((button) => button.setButtonText("Cancel").onClick(() => this.close())).addButton(
      (button) => button.setButtonText("Reset to defaults").setWarning().onClick(() => {
        this.close();
        this.confirm();
      })
    );
...
async function editFrontMatter(app, path, edit) {
  ...
  if (!(file instanceof import_obsidian10.TFile)) throw new Error(`${path} is not a file.`);
...
function withDefaultPatterns(patterns) {
  ...
  for (const [key2, defaults] of Object.entries(toppedUp)) {
  ...
  for (const [key2, defaults] of Object.entries(DEFAULT_MATCH_PATTERNS)) {
    const own = patterns[key2] ?? [];
    merged[key2] = own.length ? own : [...defaults];
  }
  return merged;
}
...
var RuleListModal = class extends import_obsidian11.Modal {
  ...
  draw() {
    ...
    for (const rule of this.rules) {
      ...
      const setting = new import_obsidian11.Setting(contentEl).setName(rule.name).setDesc(`${this.describe(rule)} \u2014 matches ${matches} transaction${matches === 1 ? "" : "s"}`);
      setting.addToggle(
    ...
    on(apply, "click", async () => {
      ...
      new import_obsidian11.Notice(`Updated ${updated} transaction${updated === 1 ? "" : "s"}.`);
      this.draw();
    });
  }
  describe(rule) {
...
var RuleEditModal = class extends import_obsidian11.Modal {
  ...
  draw() {
    ...
    new import_obsidian11.Setting(contentEl).setName("Name").addText(
...
var FinanceAutomationSettingTab = class extends import_obsidian12.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    new import_obsidian12.Setting(containerEl).setName("Budget folders").setDesc(
      `Write the ${VAULT_ROOT} tree \u2014 inbox, transactions, settings notes, two accounts and a starting set of categories. Every file it writes is reset to its default, so budgets, colours, card endings and learned keywords on those notes are replaced \u2014 it lists them and asks first. Your transactions are never touched.`
    ).addButton(
      (button) => button.setButtonText("Create folders").onClick(async () => {
    ...
    new import_obsidian12.Setting(containerEl).setName("Process when Obsidian starts").setDesc("Parse pending notes shortly after opening the vault.").addToggle(
    ...
    new import_obsidian12.Setting(containerEl).setName("Watch transaction notes").setDesc("Run automatically shortly after a transaction note is created or changed.").addToggle(
...
var PeriodPicker = class {
  ...
  render(container) {
    ...
    (0, import_obsidian13.setIcon)(back, "chevron-left");
    ...
    (0, import_obsidian13.setIcon)(forward, "chevron-right");
...
var TransactionRow = class {
  ...
  render(container) {
    ...
    (0, import_obsidian15.setIcon)(glyph, categoryIcon(record.category, this.categories));
...
var TransactionsTab = class {
  ...
  renderAddButton(container) {
    ...
    (0, import_obsidian17.setIcon)(button, "plus");
    ...
    menu.addItem(
      (item) => item.setTitle(record.excluded ? "Include in calculations" : "Exclude from calculations").setIcon(record.excluded ? "eye" : "eye-off").onClick(async () => {
        try {
          ...
          new import_obsidian17.Notice(`Could not update the transaction: ${error.message}`);
        }
      })
    );
    menu.addSeparator();
      ...
      menu.addItem(
        (item) => item.setTitle(name).setChecked(record.category === name).onClick(async () => {
          try {
            ...
            new import_obsidian17.Notice(`Could not set the category: ${error.message}`);
          }
        })
      );
    }
...
var AccountEditorModal = class extends import_obsidian18.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian18.Setting(contentEl).setName("Name").setDesc("The name on the note and on every transaction filed to this account.").addText((text) => {
    ...
    new import_obsidian18.Setting(contentEl).setName("Bank").addText(
      (text) => text.setPlaceholder("CIB").setValue(this.draft.institution).onChange((value) => {
        this.draft.institution = value;
      })
    );
    new import_obsidian18.Setting(contentEl).setName("Type").addDropdown((dropdown) => {
    ...
    new import_obsidian18.Setting(contentEl).setName("Currency").addText(
      (text) => text.setPlaceholder("EGP").setValue(this.draft.currency).onChange((value) => {
        this.draft.currency = value;
      })
    );
    new import_obsidian18.Setting(contentEl).setName("Card endings").setDesc(
    ...
    new import_obsidian18.Setting(contentEl).setName("Other names").setDesc("Separated by commas. Wordings the bank uses for this account in its messages.").addText(
      (text) => text.setPlaceholder("bank, current account").setValue(this.draft.aliases).onChange((value) => {
        this.draft.aliases = value;
      })
    );
    new import_obsidian18.Setting(contentEl).setName("Starting balance").setDesc("What the account held on the starting date. Every transaction since is added to it.").addText((text) => {
    ...
    new import_obsidian18.Setting(contentEl).setName("Statement balance").setDesc(
    ...
    new import_obsidian18.Setting(contentEl).setName("In use").setDesc("A closed account keeps its transactions but leaves the accounts tab.").addToggle(
      (toggle) => toggle.setValue(this.draft.active).onChange((value) => {
        this.draft.active = value;
      })
    );
    new import_obsidian18.Setting(contentEl).setName("Count towards net worth").addToggle(
  ...
  async save() {
    ...
    try {
      if (!this.account) {
        ...
        new import_obsidian18.Notice(`Added ${name}.`);
        this.close();
        return;
      }
      ...
      new import_obsidian18.Notice(`Could not save the account: ${error.message}`, 1e4);
    }
  }
};
...
var AccountsTab = class {
  ...
  render(container) {
    ...
    for (const item of [...balances].sort((a, b) => b.balance - a.balance)) {
      ...
      (0, import_obsidian19.setIcon)(icon, TYPE_ICONS[item.account.accountType] ?? "wallet");
...
var CategoryNameModal = class extends import_obsidian20.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian20.Setting(contentEl).setName("Name").setDesc("This is the name on the note and on every transaction filed here.").addText((text) => {
...
var DeleteCategoryModal = class extends import_obsidian20.Modal {
  constructor(app, prompt) {
    super(app);
    this.prompt = prompt;
  ...
  onOpen() {
    ...
    if (this.prompt.held) {
      new import_obsidian20.Setting(contentEl).setName(`Move ${this.prompt.held} transaction${this.prompt.held === 1 ? "" : "s"} to`).addDropdown((dropdown) => {
...
var MerchantsTab = class {
  ...
  render(container) {
    ...
    if (!all.length) {
      renderEmptyState(
        ...
        "store",
        "No names yet",
        records.length ? "None of these transactions name anyone yet. Try reading the stored messages again, widen the period, or add your bank's wording to merchant_patterns in Budget/Settings/sms_patterns.md." : "No transactions match these filters. Try a different period."
      );
  ...
  renderFillButton(container, unnamed) {
    ...
    button.addEventListener("click", () => {
      ...
      void this.plugin.fillMissingCounterparties().then((filled) => {
        new import_obsidian21.Notice(`Named the other side of ${filled} transaction(s).`, 6e3);
      }).catch((error) => {
        new import_obsidian21.Notice(`Could not read the messages: ${error.message}`, 1e4);
      }).finally(() => {
  ...
  renderRow(list, item, categories) {
    ...
    (0, import_obsidian21.setIcon)(open, "chevron-right");
    ...
    new CategoryNameModal(this.plugin.app, {
      ...
      onSubmit: async (name) => {
        try {
          ...
          new import_obsidian21.Notice(`Could not create the category: ${error.message}`, 1e4);
          return;
        }
...
var CategoriesTab = class {
  ...
  render(container) {
    ...
    if (this.rulesError) {
      ...
      problem.createEl("p", {
        text: "Fix Budget/Settings/Categories/rules.md, then reopen the Budget view. Colours and budgets can still be changed; keywords cannot, because saving them would discard the rules that failed to load."
      });
    }
    if (!shown.length) {
      renderEmptyState(
    ...
    add.addEventListener("click", () => {
      new CategoryNameModal(this.plugin.app, {
        ...
        onSubmit: async (name) => {
          try {
            await this.plugin.createCategory(name);
            new import_obsidian22.Notice(`Added ${name}.`);
          } catch (error) {
            new import_obsidian22.Notice(`Could not create the category: ${error.message}`, 1e4);
          }
        }
      }).open();
    });
  ...
  renderRow(list, row, map) {
    ...
    const paintGlyph = (fill, icon) => {
      ...
      (0, import_obsidian22.setIcon)(glyph, icon);
    };
  ...
  renderTools(body, row) {
    ...
    if (!row.note) {
      ...
      on(create, "click", async () => {
        ...
        try {
          ...
          new import_obsidian22.Notice(`Added ${row.name}.`);
        } catch (error) {
          new import_obsidian22.Notice(`Could not create the category: ${error.message}`, 1e4);
    ...
    (0, import_obsidian22.setIcon)(rename, "pencil");
    ...
    (0, import_obsidian22.setIcon)(remove, "trash");
    remove.addEventListener("click", () => {
    ...
    for (const color of CATEGORY_PALETTE) {
      ...
      on(swatch, "click", async () => {
        try {
          ...
          new import_obsidian22.Notice(`Could not save the colour: ${error.message}`);
        }
      });
    }
    new import_obsidian22.Setting(editor).setName("Icon").addDropdown((dropdown) => {
      ...
      dropdown.onChange(async (value) => {
        try {
          ...
          new import_obsidian22.Notice(`Could not save the icon: ${error.message}`);
        }
      });
    });
    new import_obsidian22.Setting(editor).setName("Monthly budget").setDesc(category.currency).addText((text) => {
      text.inputEl.type = "number";
      text.inputEl.inputMode = "decimal";
      ...
      on(text.inputEl, "change", async () => {
        ...
        if (parsed !== null && !Number.isFinite(parsed)) {
          new import_obsidian22.Notice("That budget is not a number.");
          return;
        }
        try {
          ...
          new import_obsidian22.Notice(`Could not save the budget: ${error.message}`);
        }
      });
    });
  ...
  renderKeywords(editor, category) {
    ...
    new import_obsidian22.Setting(editor).setName("Keywords").setDesc("Separated by commas. A message containing one of them files itself here.").addTextArea((area) => {
      ...
      on(area.inputEl, "change", async () => {
        ...
        try {
          ...
          new import_obsidian22.Notice(`Could not save the keywords: ${error.message}`);
        }
      });
    });
  }
  ...
  render(container) {
    ...
    on(exportButton, "click", async () => {
      try {
        ...
        new import_obsidian23.Notice(`Exported ${records.length} transactions to ${path}.`);
      } catch (error) {
        new import_obsidian23.Notice(`Export failed: ${error.message}`);
      }
    });
  }
...
var AddTransactionModal = class extends import_obsidian25.Modal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
    this.draft = {
  ...
  onOpen() {
    ...
    new import_obsidian25.Setting(contentEl).setName("Amount").addText((text) => {
      text.inputEl.type = "number";
      text.inputEl.inputMode = "decimal";
      text.inputEl.focus();
    ...
    new import_obsidian25.Setting(contentEl).setName("Type").addDropdown((dropdown) => {
    ...
    new import_obsidian25.Setting(contentEl).setName("Account").addDropdown((dropdown) => {
    ...
    const toAccountSetting = new import_obsidian25.Setting(contentEl).setName("To account").addDropdown((dropdown) => {
      dropdown.addOption("", "\u2014");
    ...
    new import_obsidian25.Setting(contentEl).setName("Category").addDropdown((dropdown) => {
    ...
    const partySetting = new import_obsidian25.Setting(contentEl).setName(ROLE_LABELS[roleForType(this.draft.type)]).addText(
    ...
    new import_obsidian25.Setting(contentEl).setName("Date").addText((text) => {
      text.inputEl.type = "date";
    ...
    new import_obsidian25.Setting(contentEl).setName("Note").addTextArea(
  ...
  async save() {
    ...
    if (!Number.isFinite(amount) || amount === 0) {
      new import_obsidian25.Notice("Enter an amount.");
      return;
    }
    if (!this.draft.account) {
      new import_obsidian25.Notice("Choose an account.");
      return;
    }
    ...
    try {
      ...
      new import_obsidian25.Notice(`Added ${file.basename}.`);
      this.close();
    } catch (error) {
      new import_obsidian25.Notice(`Could not add the transaction: ${error.message}`);
    }
  }
};
...
var TransactionSheet = class extends import_obsidian26.Modal {
  ...
  onOpen() {
    ...
    const accounts = this.plugin.index.accounts().map((account) => account.name).sort();
    const categories = this.plugin.index.categories().map((category) => category.name).sort();
    new import_obsidian26.Setting(contentEl).setName("Amount").addText(
    ...
    new import_obsidian26.Setting(contentEl).setName("Date").addText((text) => {
      text.inputEl.type = "date";
      text.setValue(this.draft.date).onChange((value) => {
        this.draft.date = value;
      });
    ...
    new import_obsidian26.Setting(contentEl).setName("Type").addDropdown((dropdown) => {
    ...
    new import_obsidian26.Setting(contentEl).setName("Category").addDropdown((dropdown) => {
      const options = categories.length ? categories : ["Uncategorized"];
    ...
    const partySetting = new import_obsidian26.Setting(contentEl).setName(ROLE_LABELS[roleForType(this.draft.type)]).addText(
      (text) => text.setValue(this.draft.counterparty).onChange((value) => {
        this.draft.counterparty = value;
      })
    );
    new import_obsidian26.Setting(contentEl).setName("Exclude from calculations").setDesc("The transaction stays in the list but counts towards nothing.").addToggle(
    ...
    const reasonSetting = new import_obsidian26.Setting(contentEl).setName("Reason").addText(
  ...
  accountSetting(container, label, accounts, field) {
    new import_obsidian26.Setting(container).setName(label).addDropdown((dropdown) => {
      dropdown.addOption("", "\u2014");
  ...
  async save() {
    ...
    if (amount !== null && !Number.isFinite(amount)) {
      new import_obsidian26.Notice("That amount is not a number.");
      return;
    }
    ...
    try {
      ...
      new import_obsidian26.Notice(`Could not save: ${error.message}`);
    }
  }
};

// src/main.ts
var FinanceAutomationPlugin = class extends import_obsidian27.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        else if (!await this.app.vault.adapter.exists((0, import_obsidian27.normalizePath)(INBOX_DIR))) {
          new import_obsidian27.Notice(
            `Budget: ${INBOX_DIR} does not exist yet. Run "Create budget folders" first.`,
            8e3
          );
        } else new import_obsidian27.Notice(`Budget: no messages waiting in ${INBOX_DIR}.`);
      }
    });
    this.addCommand({
      id: "add-transaction",
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian27.Notice(`Exported ${records.length} transactions to ${path}.`);
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian27.Notice(`Budget: named the other side of ${updated} transaction(s).`, 6e3);
      }
    });
    this.addCommand({
      id: "list-merchants",
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian27.Notice(`Budget: updated ${updated} transaction(s).`);
      }
    });
  ...
  async handleCaptureLink(params) {
    try {
      ...
      new import_obsidian27.Notice(`Budget: captured ${file.path}.`, 5e3);
    } catch (error) {
      console.error("Ultra Budget Tracker: capture link failed", error);
      new import_obsidian27.Notice(`Budget capture failed: ${error.message}`, 1e4);
    }
  }
  /**
  ...
  async captureInbox() {
    ...
    if (summary) new import_obsidian27.Notice(`Budget: ${summary}.`, 6e3);
  ...
  async runFinance(showNotice) {
    if (this.running) {
      this.queued = true;
      if (showNotice) new import_obsidian27.Notice("Budget processing is already running; another pass is queued.");
      return;
    }
    this.running = true;
    this.setStatus("running\u2026");
    if (showNotice) new import_obsidian27.Notice("Budget: processing\u2026");
    try {
      if (await this.captureInbox()) this.queued = true;
      const updated = await this.processPending();
      this.setStatus("ready");
      if (showNotice) new import_obsidian27.Notice(`Budget: updated ${updated} transaction(s).`, 6e3);
    } catch (error) {
      this.setStatus("error");
      console.error("Ultra Budget Tracker: processing failed", error);
      new import_obsidian27.Notice(`Budget processing failed: ${error.message}`, 1e4);
    } finally {
      this.running = false;
      if (this.queued) {
        this.queued = false;
  ...
  async loadParserInputs() {
    const [config, patterns, accountsJson, categories] = await Promise.all([
      ...
      this.loadPatterns(),
      loadVaultJson(this.app, ACCOUNTS_CONFIG_PATH, { accounts: [] }),
